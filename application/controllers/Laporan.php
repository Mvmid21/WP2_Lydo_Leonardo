...
class Laporan extends CI_Controller
{
    ...
    { 
        {
            $this->load->library('dompdf_gen');
            $data['anggota'] = $this->ModelAnggota->getAnggota()->result_array();
            $this->load->view('anggota/laporan_anggota_pdf', $data);
    ...
    public function laporan_buku_excel()

    {
        $data = array('title' => 'Laporan Buku', 'buku' => $this->ModelBuku->getBuku()->result_array());
        $this->load->view('buku/laporan_buku_excel', $data);
    }
    public function laporan_anggota_excel()

    {
        $data = array('title' => 'Laporan Anggota', 'anggota' => $this->ModelAnggota->getAnggota()->result_array());
        $this->load->view('anggota/laporan_anggota_excel', $data);
    }
    ...
    public function laporan_anggota()
    {
        ...
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['anggota'] = $this->ModelAnggota->getAnggota()->result_array();
        $data['kategori'] = $this->ModelAnggota->getKategori()->result_array();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
